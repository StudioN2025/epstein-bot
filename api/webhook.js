  ...
  if (req.method === 'POST') {
    try {
      ...
      // ========== ПОДВАЛЫ СТИВЕНА ХОКИНГА ==========
        ...
        const requiredBasements = user.children || 0;
        const hasEnough = (user.basements || 0) >= requiredBasements;
        const canBuyNewChild = (user.basements || 0) > (user.children || 0);
        
        let statusEmoji = hasEnough ? '✅' : '❌';
        let statusText = hasEnough ? 'Достаточно' : 'НЕ хватает!';
        
        await sendMessage(BOT_TOKEN, chatId,
          `🏚️ *ПОДВАЛЫ СТИВЕНА ХОКИНГА* 🏚️\n\n` +
          `🧼 Цена одного подвала: ${BASEMENT_COST} мыла\n` +
          `📊 Твои подвалы: ${user.basements || 0}\n` +
          `👶 Твои дети: ${user.children || 0}\n` +
          `🏚️ Необходимо подвалов для текущих детей: ${requiredBasements}\n` +
          `📌 Статус: ${statusEmoji} ${statusText}\n\n` +
          `${canBuyNewChild ? '✅ Ты можешь купить нового ребенка! /buychild' : '❌ Тебе нужно больше подвалов! /buybasement'}\n\n` +
        ...
        if (user.balance >= BASEMENT_COST) {
          ...
          const requiredForNextChild = (user.children || 0) + 1 - (user.basements || 0);
          const canBuyChild = (user.basements || 0) > (user.children || 0);
          ...
          if (canBuyChild) {
            message += `✅ Теперь ты можешь купить ребенка! /buychild`;
          } else {
            message += `⚠️ Нужно еще ${Math.abs(requiredForNextChild)} подвалов для следующего ребенка`;
          }
      ...
      // ========== ОБЫЧНЫЕ КОМАНДЫ ==========
        ...
        // Проверка: нужно минимум столько же подвалов, сколько будет детей
        if (userBasements <= userChildren) {
          const needed = userChildren + 1 - userBasements;
          await sendMessage(BOT_TOKEN, chatId,
            `❌ ${username}, у тебя не хватает подвалов для ребенка!\n\n` +
            `🏚️ Твои подвалы: ${userBasements}\n` +
            `👶 Твои дети: ${userChildren}\n` +
            `📌 Нужно минимум ${userChildren + 1} подвалов, чтобы купить ${userChildren + 1}-го ребенка\n` +
            `⚠️ Осталось купить подвалов: ${needed}\n\n` +
            `/buybasement — купить подвал (${BASEMENT_COST} 🧼)`);
          return res.status(200).json({ ok: true });
        }
        ...
        const userBasements = user.basements || 0;
        const userChildren = user.children || 0;
        const canBuyNew = userBasements > userChildren;
        ...
        if (sorted.length === 0 || sorted[0].basements === 0) {
          ...
          sorted.forEach((u, i) => {
            if (u.basements > 0) {
              reply += `${i+1}. ${u.username} — ${u.basements} 🏚️ (👶 ${u.children || 0})\n`;
            }
          });
          await sendMessage(BOT_TOKEN, chatId, reply);
        }
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `/buybasement — купить подвал (${BASEMENT_COST} 🧼)\n` +
          `/buychild — купить ребенка (${CHILD_COST} 🧼, нужен подвал на каждого)\n` +
