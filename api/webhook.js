    ...
    try {
      ...
      // ========== ИВЕНТ СВО ==========
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `• Дети в подвалах защищают (1 ребенок = защищает 1 подвал)\n` +
        ...
        // Расчет защиты: дети в подвалах защищают
        const targetChildren = targetUser.children || 0;
        const targetBasements = targetUser.basements || 0;
        const maxProtectors = targetBasements * CHILDREN_PER_BASEMENT;
        const actualProtectors = Math.min(targetChildren, maxProtectors);
        
        let attackers = amount;
        let defenders = actualProtectors;
        
        let message = `⚔️ *АТАКА!* ⚔️\n\n` +
          `${username} атакует @${targetName} с ${attackers} мобилизованными детьми!\n` +
          `🛡️ Защитники: ${defenders} детей в подвалах\n\n`;
        ...
        if (attackers > defenders) {
          ...
          const capturedBasements = Math.floor(survivors / CHILDREN_PER_BASEMENT);
          ...
          // Захват подвалов
          const actualCapture = Math.min(capturedBasements, targetBasements);
          
          if (actualCapture > 0) {
            targetUser.basements -= actualCapture;
            user.capturedBasements = (user.capturedBasements || 0) + actualCapture;
            message += `💥 *ПОБЕДА!* 💥\n` +
              `🗡️ Убито защитников: ${killedDefenders}\n` +
              `💀 Потери атакующих: ${killedAttackers}\n` +
              `🏚️ Захвачено подвалов: ${actualCapture}\n` +
              `💰 Каждый захваченный подвал приносит ${BASEMENT_CAPTURE_REWARD} 🧼/час!\n\n`;
          } else {
            message += `💥 *ПОБЕДА, НО ПОДВАЛОВ НЕТ!* 💥\n` +
              `🗡️ Убито защитников: ${killedDefenders}\n` +
              `💀 Потери атакующих: ${killedAttackers}\n` +
              `❌ У ${targetName} не осталось подвалов для захвата!\n\n`;
          }
          
          if (remainingAttackers > 0) {
            message += `⚔️ Осталось атакующих: ${remainingAttackers} (возвращаются домой)`;
          }
          
          // Обновляем данные
          targetUser.children -= killedDefenders;
          targetUser.basements = Math.max(0, targetUser.basements);
        ...
        // Списываем мобилизованных атакующего
        user.mobilized -= killedAttackers;
