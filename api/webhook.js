  ...
  if (req.method === 'POST') {
    try {
        ...
        if (cbData.startsWith('accept_')) {
          ...
          for (const [id, duel] of Object.entries(duels)) {
            if (duel.player1Id === userId || duel.player2Id === userId) {
              await sendMessage(BOT_TOKEN, chatId, `❌ ${escapeMarkdown(username)} отменил дуэль.`);
          ...
          if (duel.turn !== userId) {
            ...
            await answerCallback(callback.id, `⏳ Сейчас ход ${escapeMarkdown(waitingName)}!`);
            return res.status(200).json({ ok: true });
          }
          ...
          if (action === 'aim') {
            ...
            resultText = `🎯 ${escapeMarkdown(username)} прицелился! Точность: ${Math.min(20 + aimBonus, 50)}%`;
            duel.turn = isPlayer1 ? duel.player2Id : duel.player1Id;
          }
          else if (action === 'break') {
            if (isPlayer1) {
              duel.aim2 = 0;
              resultText = `🔫 ${escapeMarkdown(username)} сбил прицел у ${escapeMarkdown(duel.player2Name)}!`;
            } else {
              duel.aim1 = 0;
              resultText = `🔫 ${escapeMarkdown(username)} сбил прицел у ${escapeMarkdown(duel.player1Name)}!`;
            }
            duel.turn = isPlayer1 ? duel.player2Id : duel.player1Id;
          }
            ...
            resultText = `🎲 ${escapeMarkdown(username)} стреляет! Точность: ${hitChance}%, выпало: ${hitRoll.toFixed(1)}%\n\n`;
            if (hit) {
              resultText += `💥 ПОПАДАНИЕ!\n${escapeMarkdown(username)} попал в ${escapeMarkdown(targetName)}!\n`;
              if (targetData.balance < bet) {
                ...
                resultText += `😵 У ${escapeMarkdown(targetName)} было ${oldBalance} мыла! Не хватило ${bet} мыла!\n🔇 МУТ на 1 минуту!`;
                shooterData.balance += oldBalance;
              } else {
                targetData.balance -= bet;
                shooterData.balance += bet;
                resultText += `🧼 ${escapeMarkdown(username)} забрал ${bet} мыла!\n📊 ${escapeMarkdown(username)}: ${shooterData.balance} 🧼, ${shooterData.children || 0} 👶\n📊 ${escapeMarkdown(targetName)}: ${targetData.balance} 🧼, ${targetData.children || 0} 👶`;
              }
              resultText += `\n\n🏆 ПОБЕДИТЕЛЬ: ${escapeMarkdown(username)}`;
              ...
              resultText += `💨 ПРОМАХ!\n${escapeMarkdown(username)} промахнулся!`;
      ...
      if (user.mutedUntil && user.mutedUntil > Math.floor(Date.now() / 1000)) {
        ...
        await sendMessage(BOT_TOKEN, chatId, `🔇 ${escapeMarkdown(username)}, мут ${Math.ceil(remaining / 60)} мин!`);
        return res.status(200).json({ ok: true });
      }
      ...
      if (isAdmin && cleanText.startsWith('/addsoap')) {
        ...
        if (!targetId) {
          await sendMessage(BOT_TOKEN, chatId, `❌ Не найден @${escapeMarkdown(targetUsername)}`);
          return res.status(200).json({ ok: true });
        }
        ...
        await saveData(data);
        await sendMessage(BOT_TOKEN, chatId, `✅ Админ ${escapeMarkdown(username)} добавил ${amount} 🧼 @${escapeMarkdown(targetUsername)}\n📊 Теперь: ${targetUser.balance} 🧼`);
      }
        ...
        data.users[targetId] = targetUser;
        await saveData(data);
        await sendMessage(BOT_TOKEN, chatId, `✅ Админ ${escapeMarkdown(username)} снял ${amount} 🧼 у @${escapeMarkdown(targetUsername)}\n📊 Теперь: ${targetUser.balance} 🧼`);
      }
      ...
      // ========== ОБЫЧНЫЕ КОМАНДЫ ==========
        ...
        if (user.balance >= CHILD_COST) {
          ...
          data.users[userId] = user;
          await saveData(data);
          await sendMessage(BOT_TOKEN, chatId, `👶 ${escapeMarkdown(username)} купил ребенка!\n🧼 ${user.balance} мыла\n👶 ${user.children} детей\n📈 Каждый ребенок приносит ${CHILD_INCOME} мыло в час!`);
        } else {
        ...
        await sendMessage(BOT_TOKEN, chatId, `👶 *ДЕТИ ${escapeMarkdown(username)}* 👶\n\n🧼 Мыла: ${user.balance}\n👶 Детей: ${user.children}\n📈 Пассивный доход: ${hourlyIncome} 🧼/час\n\n/buychild - 100 мыла = 1 ребенок`);
      }
        ...
        const normalizedUsers = users.map(u => ({
          ...
          children: typeof u.children === 'number' ? u.children : 0
        }));
        const sorted = normalizedUsers.sort((a, b) => b.balance - a.balance).slice(0, 10);
        ...
        if (sorted.length === 0 || sorted[0].balance === 0) {
          ...
          sorted.forEach((u, i) => {
            ...
            reply += `${i+1}. ${escapeMarkdown(u.username)} — ${u.balance} 🧼 (👶 ${u.children || 0}, +${childIncome}/ч)\n`;
          });
          await sendMessage(BOT_TOKEN, chatId, reply);
        }
      }
        ...
        const sorted = normalizedUsers.sort((a, b) => (b.children || 0) - (a.children || 0)).slice(0, 10);
        ...
        if (sorted.length === 0 || sorted[0].children === 0) {
          ...
          sorted.forEach((u, i) => {
            if (u.children > 0) {
              reply += `${i+1}. ${escapeMarkdown(u.username)} — ${u.children} 👶 (${u.children * CHILD_INCOME} 🧼/ч)\n`;
            }
          });
          await sendMessage(BOT_TOKEN, chatId, reply);
        }
        ...
        if (promo.usedBy.includes(userId)) {
          await sendMessage(BOT_TOKEN, chatId, `❌ ${escapeMarkdown(username)}, ты уже активировал этот промокод!`);
          return res.status(200).json({ ok: true });
        }
        ...
        if (opponent.balance < bet) {
          await sendMessage(BOT_TOKEN, chatId, `❌ У @${escapeMarkdown(opponentName)} не хватает мыла! Есть: ${opponent.balance}, нужно: ${bet}`);
          return res.status(200).json({ ok: true });
        }
        ...
        if (user.lastFarm && (nowSec - user.lastFarm) < 3600) {
          ...
          if (Math.random() * 100 < PIDIDI_STEAL_CHANCE) {
            ...
            if (user.balance - stolen <= 0) {
              user.balance = 0;
              message = `😡👶 ПИДИДИ УКРАЛ ВСЁ!\n${escapeMarkdown(username)}, осталось 0 мыла!`;
            } else {
              user.balance -= stolen;
        ...
        const hourlyIncome = (user.children || 0) * CHILD_INCOME;
        await sendMessage(BOT_TOKEN, chatId, `📊 *${escapeMarkdown(username)}*\n\n🧼 Мыла: ${user.balance}\n👶 Детей: ${user.children || 0}\n📈 Пассивный доход: ${hourlyIncome} 🧼/час\n\n/buychild - 100 мыла = ребенок`);
      }
