  ...
  if (req.method === 'POST') {
    try {
      ...
      let user = data.users[userId] || { 
        ...
        mobilized: 0, capturedBasements: 0, lastCapturedIncome: Date.now()
      };
      ...
      // ========== ИВЕНТ СВО ==========
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `🎯 *Механика:*\n` +
          `• Мобилизуй детей за ${MOBILIZATION_COST} 🧼 за ребенка\n` +
          `• Мобилизованные дети атакуют врага\n` +
          `• Каждый подвал защищают ДО 10 детей (сколько есть)\n` +
          `• При атаке сначала гибнут защитники, потом захватываются подвалы\n` +
          `• Захваченный подвал приносит ${BASEMENT_CAPTURE_REWARD} 🧼/час\n\n` +
          `/mobilize [количество] — мобилизовать детей\n` +
          `/attack @user [количество] — атаковать пользователя\n` +
          `/myarmy — моя армия\n` +
          `/captured — захваченные подвалы`);
      }
        ...
        if (userChildren < amount) {
          await sendMessage(BOT_TOKEN, chatId, `❌ У тебя нет ${amount} детей! У тебя: ${userChildren} 👶`);
          return res.status(200).json({ ok: true });
        }
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `/attack @user ${amount} — отправить в атаку`);
      }
        ...
        // Расчет защиты: каждый подвал защищают ДО 10 детей (сколько есть)
        const targetBasements = targetUser.basements || 0;
        const targetChildren = targetUser.children || 0;
        
        // Распределяем детей по подвалам (макс 10 на подвал)
        let defendersPerBasement = [];
        let remainingChildren = targetChildren;
        for (let i = 0; i < targetBasements; i++) {
          let defenders = Math.min(remainingChildren, CHILDREN_PER_BASEMENT);
          defendersPerBasement.push(defenders);
          remainingChildren -= defenders;
        }
        ...
        // Атака по подвалам последовательно
        for (let i = 0; i < defendersPerBasement.length; i++) {
          if (attackers <= 0) break;
          
          const defenders = defendersPerBasement[i];
          ...
          if (attackers > defenders) {
            ...
            defendersPerBasement[i] = 0;
          } else {
            killedAttackers += attackers;
            killedDefenders += attackers;
            defendersPerBasement[i] -= attackers;
            attackers = 0;
            break;
          }
        }
        ...
        let message = `⚔️ *АТАКА!* ⚔️\n\n` +
          `${username} атакует @${targetName} с ${amount} мобилизованными детьми!\n` +
          `🛡️ У ${targetName}: ${targetBasements} подвалов, ${targetChildren} детей\n\n`;
        
        if (capturedBasements > 0) {
          const remainingBasements = targetBasements - capturedBasements;
          targetUser.basements = remainingBasements;
          
          let newChildren = 0;
          for (let i = 0; i < defendersPerBasement.length; i++) {
            newChildren += defendersPerBasement[i];
          }
          targetUser.children = newChildren;
          
          user.capturedBasements = (user.capturedBasements || 0) + capturedBasements;
          ...
          message += `🛡️ *АТАКА ОТБИТА!* 🛡️\n` +
            ...
            `🏚️ У ${targetName} осталось подвалов: ${targetUser.basements}\n` +
            `👶 Осталось защитников: ${targetUser.children}`;
        }
        
        if (attackers > 0) {
          message += `\n\n⚔️ Осталось атакующих: ${attackers} (возвращаются домой)`;
        }
        ...
        await sendMessage(BOT_TOKEN, chatId,
          `⚔️ *АРМИЯ ${username}* ⚔️\n\n` +
          `👶 Мобилизовано детей: ${user.mobilized || 0}\n` +
          `🏚️ Своих подвалов: ${user.basements || 0}\n` +
          `👶 Детей в подвалах: ${Math.min(user.children || 0, (user.basements || 0) * CHILDREN_PER_BASEMENT)}\n` +
          `📊 Баланс: ${user.balance} 🧼\n\n` +
          `/mobilize [количество] — мобилизовать детей\n` +
          `/attack @user [количество] — атаковать`);
      }
      
      else if (cleanText === '/captured') {
        const capturedIncome = (user.capturedBasements || 0) * BASEMENT_CAPTURE_REWARD;
        await sendMessage(BOT_TOKEN, chatId,
          `🏚️ *ЗАХВАЧЕННЫЕ ПОДВАЛЫ* 🏚️\n\n` +
          `🏚️ Захвачено подвалов: ${user.capturedBasements || 0}\n` +
          `💰 Доход: ${capturedIncome} 🧼/час\n` +
          `📈 Пассивный доход от захваченных подвалов начисляется автоматически!`);
      }
      
      // ========== СЕКРЕТНАЯ ЯДЕРНАЯ БОМБА (НИКТО НЕ ЗНАЕТ) ==========
      else if (cleanText === '/nuke') {
        const nowTime = Date.now();
        if (nowTime < NUKE_ACTIVATE_DATE) {
          // Ничего не отвечаем — секрет!
          return res.status(200).json({ ok: true });
        }
        ...
        // Тихое сообщение только атакующему
        await sendMessage(BOT_TOKEN, chatId,
          `💥 *СЕКРЕТНОЕ ОРУЖИЕ АКТИВИРОВАНО* 💥\n\n` +
          `🎯 Цель: @${targetName}\n` +
          `💣 Уничтожено подвалов: ${destroyedBasements}\n` +
          `👶 Уничтожено детей: ${destroyedChildren}\n` +
          `🏚️ Потеряно захваченных подвалов: ${destroyedCaptured}\n\n` +
      ...
      // ========== ОБЫЧНЫЕ КОМАНДЫ ==========
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `📈 Доход от захваченных подвалов: ${capturedIncome} 🧼/час\n\n` +
          `/buybasement [количество] — купить подвалы (${BASEMENT_COST} 🧼/шт)\n` +
