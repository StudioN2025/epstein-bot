...
const FREE_COST = 200; // стоимость освобождения одного подвала
...
// Функция для добавления захваченного подвала с деталями
...
// Функция для удаления захваченного подвала
  ...
  if (req.method === 'POST') {
    try {
      ...
      // ========== ИВЕНТ СВО ==========
        ...
        const defenders = targetUser.mobilized || 0;
        ...
        if (attackers > defenders) {
          ...
          // Сначала пытаемся отбить свои подвалы, если они у атакуемого
          if (targetUser.capturedBasementsDetails && targetUser.capturedBasementsDetails.length > 0) {
            ...
            if (myCapturedFromTarget && myCapturedFromTarget.count > 0) {
              const canRecapture = Math.min(myCapturedFromTarget.count, Math.floor(survivors / CHILDREN_PER_BASEMENT));
              if (canRecapture > 0) {
          ...
          const remainingSurvivors = survivors - (recapturedBasements * CHILDREN_PER_BASEMENT);
          if (remainingSurvivors > 0) {
            const targetBasements = targetUser.basements || 0;
            capturedBasements = Math.min(Math.floor(remainingSurvivors / CHILDREN_PER_BASEMENT), targetBasements);
          ...
          const remainingAttackers = (remainingSurvivors > 0 ? remainingSurvivors % CHILDREN_PER_BASEMENT : survivors % CHILDREN_PER_BASEMENT);
        ...
        // Проверяем, есть ли у цели захваченные подвалы, принадлежащие атакующему
      ...
      // ========== СЕКРЕТНАЯ ЯДЕРНАЯ БОМБА ==========
        ...
        await sendMessage(BOT_TOKEN, chatId,
          ...
          `👶 Уничтожено обычных детей: ${destroyedChildren}\n` +
          `⚔️ Уничтожено мобилизованных: ${destroyedMobilized}\n` +
          `🏚️ Потеряно захваченных подвалов: ${destroyedCaptured}\n\n` +
          `🔒 Эта информация останется между нами...`);
      }
      
      // ========== ОБЫЧНЫЕ КОМАНДЫ (buychild, children, top, и т.д.) ==========
