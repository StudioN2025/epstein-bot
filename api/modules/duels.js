const { sendMessage, editMessage, deleteMessage, answerCallback, loadData, saveData } = require('./helpers');
const config = require('./config');

let duels = {};

async function handleDuelCallback(update, BOT_TOKEN, duelsObj) {
  ...
  if (cbData.startsWith('accept_')) {
    ...
    for (const [id, duel] of Object.entries(duelsObj)) {
...
async function handleDuelCommand(cleanText, rawText, user, data, BOT_TOKEN, chatId, username, duelsObj) {
  ...
  for (const duel of Object.values(duelsObj)) {
    if (duel.player1Id === userId || duel.player2Id === userId) {
  ...
  duelsObj[duelId] = { id: duelId, player1Id: userId, player1Name: username, player2Id: opponentId, player2Name: opponentName, bet: bet, status: 'waiting', turn: null, aim1: 0, aim2: 0 };
  ...
  setTimeout(() => { if (duelsObj[duelId] && duelsObj[duelId].status === 'waiting') { delete duelsObj[duelId]; sendMessage(BOT_TOKEN, chatId, `⏰ Дуэль отменена`); } }, 60000);
  return true;
}

module.exports = { handleDuelCallback, handleDuelCommand };
